...
// Main Game or Menu JavaScript
document.getElementById('minigames-btn').addEventListener('click', () => {
    const minigames = ['minigame1.html', 'cookie.html'];  // Array of minigame files
    const randomGame = minigames[Math.floor(Math.random() * minigames.length)]; // Randomly select a game
    window.location.href = randomGame; // Redirect to the chosen game
});
